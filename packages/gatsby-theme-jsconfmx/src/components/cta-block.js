...
const CtaBlock = props => {
  return (
    ...
    >
      <div className="container mx-auto text-center">{props.children}</div>
    </div>
  );
};

CtaBlock.Intro = props => (
  <IntroText dark={props.dark}>{props.children}</IntroText>
);
...
CtaBlock.Cta = props => (
  <Cta
    to={props.to}
    type={props.dark ? "lightShade" : "darkShade"}
    darkText={props.dark}
  >
    {props.children}
  </Cta>
